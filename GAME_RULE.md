...
2. **바구니 조작**
   - **좌 포즈** → 바구니가 왼쪽 구역으로 이동
   - **중앙 포즈** → 바구니가 중앙 구역으로 이동
   - **우 포즈** → 바구니가 오른쪽 구역으로 이동
...
| 포즈 이름 | 게임 내 역할 | 설명 | 예시 |
|---------|------------|------|------|
| 좌 | 바구니를 왼쪽으로 이동 | 얼굴을 왼쪽으로 돌린 상태 | 👈 |
| 우 | 바구니를 오른쪽으로 이동 | 얼굴을 오른쪽으로 돌린 상태 | 👉 |
| 중앙 | 바구니를 중앙으로 이동 | 정면을 바라본 상태 | 😊 |

> **참고**: 이 게임은 3개 포즈(좌/중앙/우)를 사용하며, 모델의 실제 라벨과 일치합니다.
...
현재 `my-model/metadata.json`의 라벨:
```json
{
  "labels": ["중앙", "좌", "우"]
}
